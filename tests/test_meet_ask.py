...
def main():
    ...
    check("first ask is #1 in its chat", ask["num"] == 1)
    ask2 = db.create_ask(chat_id, "tue", "13:00", "Second?")
    check("second ask is #2", ask2["num"] == 2)
    other = db.create_ask(99999, "mon", "12:00", "Other chat?")
    check("other chat starts at #1", other["num"] == 1)
    check("lookup by chat number", db.get_ask_by_number(chat_id, 2)["id"] == ask2["id"])
    check("number is per-chat", db.get_ask_by_number(99999, 2) is None)
    db.delete_ask(chat_id, ask["id"])
    ask3 = db.create_ask(chat_id, "wed", "14:00", "Third?")
    check("numbers never reused", ask3["num"] == 3)
    db.delete_ask(chat_id, ask2["id"])
    db.delete_ask(chat_id, ask3["id"])
    db.delete_ask(99999, other["id"])
    ask = db.create_ask(chat_id, "mon", "12:00", "Are you coming?")
